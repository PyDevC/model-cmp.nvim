...
describe("model_cmp.utils", function()
    ...
    it("Text adjusting according to content", function()
        assert.are.same("1;", utils.partial_match("return ", "return 1;", { 0, 7 }))
        assert.are.same("options", utils.partial_match("return ", "return options", { 0, 7 }))
        assert.is_nil(utils.partial_match("return ", "print(options)", { 0, 4 }))
        assert.is_nil(utils.partial_match(";", "return 0;", { 0, 1 }))
        assert.are.same("0;", utils.partial_match("return ", "return 0;", { 0, 7 }))
    end
    )
end)
